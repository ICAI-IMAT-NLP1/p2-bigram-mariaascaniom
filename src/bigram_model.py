...
def bigrams_count_to_probabilities(
    ...
    length = bigram_counts.shape[0]
    probs_tensor = torch.zeros_like(bigram_counts,dtype=torch.float32)
    total_count = [bigram_counts[row].sum().item() for row in range(bigram_counts.shape[0])]

    for i in range(bigram_counts.shape[0]):
        for j in range(bigram_counts.shape[1]):
            #conditional prob with smoothing (like excercise 1 in theory)
            cond_prob = (bigram_counts[i][j] + smooth_factor) / (total_count[i] + smooth_factor*length)
            probs_tensor[i][j] = cond_prob
    return probs_tensor
...
def calculate_log_likelihood(
    ...
    for i in range(len(processed_word)-1):
        ...
        i,j = char_to_index[bigram[0]], char_to_index[bigram[1]]
